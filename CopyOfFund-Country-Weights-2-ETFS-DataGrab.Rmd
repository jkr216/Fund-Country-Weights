---
title: "Reproducible Finance with R: Fund Country Exposure 2 ETFs"
...
world_fund_country_weights <- merge(world, emerging_market_country_weights, by = "name") 
...
world_fund_country_weights@data['emerging_market']
...
Replicate those steps for another ETF - MSCI Total International. Then we'll save our objects for use later in the Shiny app.

```{r}
international_fund <- read_csv("https://www.ishares.com/us/products/244048/ishares-core-msci-total-international-stock-etf/1467271812596.ajax?fileType=csv&fileName=IXUS_holdings&dataType=fund", skip = 11)
```
...
Those top ten holdings are begging to be analyzed against some demographic data but that's not our mission here, so we'll plow forward. Let's wrange our International ETF company level data. 
...
# How about China instead of Brazil

China_companies <- international %>%
...
Alright, that semerging_markets to have worked: we just resued our code from above to wrangle the International ETF data and we can see the companies from China. 

Now we add our International Stocks ETF data to the spatial dataframe and build a leaflet map to confirm everything worked. Again, this is a repeat of the above steps.
...
# Have a peek at our new column in the dataframe.
...
# A new leaflet object, this time shaded by how the International Stock ETF is allocated
...
infrastructure_fund <- read_csv("https://www.ishares.com/us/products/239746/ishares-global-infrastructure-etf/1467271812596.ajax?fileType=csv&fileName=IGF_holdings&dataType=fund", 
    skip = 11)
...
# Add the country weight data to the spatial dataframe.
world_fund_country_weights <- merge(world_fund_country_weights, infrastructure_country_weights, by = "name")
```
...
# Add the country weight data to the spatial dataframe.
world_fund_country_weights <- merge(world_fund_country_weights, asia_ex_japan_country_weights, by = "name")
```
...
# Add the country weight data to the spatial dataframe.
world_fund_country_weights <- merge(world_fund_country_weights, europe_country_weights, by = "name")

save(world_fund_country_weights, emerging_market_country_weights, emerging_market, international, international_country_weights, infrastructure, infrastructure_country_weights, asia_ex_japan, asia_ex_japan_country_weights, europe, europe_country_weights, file = "worldFundCountryWeights.RDat")
```
